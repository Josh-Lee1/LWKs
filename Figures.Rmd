...
outsidenew <- outside %>% 
  dplyr::group_by(Trip) %>% 
  mutate(fusnew = mean(fuscusperhundred, na.rm=TRUE)) %>%
  mutate(legnew = mean(leggadina, na.rm=TRUE)) %>%
  mutate(ratnew = mean(Rattusperhundred, na.rm=TRUE)) %>%
  mutate(smacnew = mean(Smac, na.rm=TRUE)) %>%
  mutate(scrasnew = mean(Scrassi, na.rm=TRUE)) %>%
  mutate(musnew = mean(Mus, na.rm=TRUE)) %>%
  mutate(pausnew = mean(Paustralis, na.rm=TRUE)) %>%
  mutate(phernew = mean(Pherman, na.rm=TRUE)) %>%
  mutate(pdesnew = mean(Pdesertor, na.rm=TRUE)) %>%
  mutate(mulnew = mean(mulgara, na.rm=TRUE)) %>%
  dplyr::ungroup(Trip)
...
trapsfin <- trapsnew %>% 
  ...
  tidyr::gather(Species, Capsphund, fusnew:mulnew) %>% 
  distinct()

infin <- filter(trapsfin, fence == "in")
outfin <- filter(trapsfin, fence == "out")

ggplot(infin, aes(Trip, Capsphund)) +
  geom_col(aes(position = "stack", colour = Species)) + theme_bw() + ggtitle("Inside")

ggplot(outfin, aes(Trip, Capsphund)) +
  geom_col(aes(position = "stack", colour = Species)) + theme_bw() + ggtitle("Outside")
...
ggplot(rainset, aes(Date, Monthly.Precipitation.Total..millimetres.)) + geom_col() + theme_bw()
